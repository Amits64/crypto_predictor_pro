def calculate_risk_metrics(df, pred_return, confidence, config):
    ...
    size  = kelly * confidence * config.RISK_TOLERANCE

    slp = price - 2*atr if pred_return>0 else price + 2*atr
    tp1 = price*(1 + config.TAKE_PROFIT_PCT/100) if pred_return>0 else price*(1 - config.TAKE_PROFIT_PCT/100)
...
def generate_trading_signals(df, pred_return, confidence, risk_metrics, config):
    ...
    if rsi<30 and macd>macd_sig:
        signals.append({"type":"BUY","strength":3,"reason":f"RSI oversold ({rsi:.1f}) + MACD↑"})
    if rsi>70 and macd<macd_sig:
        signals.append({"type":"SELL","strength":3,"reason":f"RSI overbought ({rsi:.1f}) + MACD↓"})
